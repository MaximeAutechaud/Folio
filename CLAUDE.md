...
## Data Model (schema_version v5)
- `positions` : id, ticker, name, asset_type (stock|crypto), currency, quantity, cost_basis
- `transactions` : position_id (FK CASCADE), type (buy|sell|swap_out|swap_in), linked_tx_id (paire swap)
...
- `settings` : key/value
- Migrations inline dans `db.ts` — v2/v3 guardées par `schema_version`, v4/v5 par existence de table dans `sqlite_master` (auto-répare les DBs mal estampillées par un ancien bug).
