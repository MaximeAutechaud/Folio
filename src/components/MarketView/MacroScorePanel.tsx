...
interface Props {
  data: MacroScoreData | undefined;
  isFetching: boolean;
  title: string;
  /** Message pendant le premier chargement. */
  loadingLabel: string;
  /** Intitulé de la section des indicateurs `contextOnly`. */
  contextLabel: string;
  footnote: string;
  /** Graphique optionnel rendu en bas du panneau déplié. */
  children?: ReactNode;
}

/**
 * Présentation d'un score de régime 0-100 et de ses indicateurs. Partagée par
 * le contexte macro actions et le contexte macro crypto : les deux produisent
 * un `MacroScoreData`, seuls les libellés changent.
 */
export function MacroScorePanel({
  data, isFetching, title, loadingLabel, contextLabel, footnote, children,
}: Props) {
  ...
  if (!data) {
    return (
      <div className={styles.root}>
        <span className={styles.loading}>{isFetching ? loadingLabel : '—'}</span>
      </div>
    );
  }

  const { score, scorePrev, trend, regime, indicators, warning } = data;
  ...
  return (
    <div className={styles.root}>
      ...
      {/* Une entrée pondérée manquante est neutralisée à 50 : le score reste
          d'allure normale, il faut donc le dire au-dessus du pli. */}
